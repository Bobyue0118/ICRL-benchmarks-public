...
def cal_gra_of_lambda_2(gamma, R_k, x, reward):
...
def cal_R_k(gamma, transition, estimated_transition, expert_policy, expert_policy_active, R_max = 1):
    R_k = 2*gamma*R_max*np.max(transition-estimated_transition)/(1-gamma)**2 + gamma*R_max*np.max(expert_policy-expert_policy_active)/(1-gamma)**2
    return R_k
